...
- 如果服务、Job、镜像 tag 或运行态版本仍是 mock，不能进入阶段 5。

### 阶段 5：发布单创建

目标：

- 发布单页面从真实环境、真实 Agent、真实服务版本来源读取数据。
...
- 如果发布单创建仍依赖 mock 环境、mock Agent、mock 服务或 mock 版本，不能进入阶段 6。

### 阶段 6：基线管理

目标：

- 基线列表、基线详情、来源环境、快照时间、服务清单全部来自真实数据。
...
- 如果基线来源仍是 mock，基线管理不能算完成，不能进入阶段 7。
...
- 如果 Agent 还不能对真实目标基础设施执行操作，部署执行不能算完成，不能进入阶段 8。

### 阶段 8：发布详情 / 部署详情

目标：

- 详情页展示真实任务状态、步骤、日志、失败原因、最终结果。
...
- 如果详情页还依赖 mock task 数据，不能进入阶段 9。

### 阶段 9：登录与权限

目标：

- 登录、用户、角色、权限、环境级授权全部改为真实数据。
...
本节保留为历史上下文，不再作为后续研发执行顺序。当前唯一有效顺序以上文“V1 研发主线路径”为准：基础资源管理、环境管理、Agent 管理与远程探测、服务与版本来源、发布单创建、基线管理、部署执行、发布/部署详情、登录与权限、清理剩余 mock。每个阶段必须接真实数据并删除该阶段 mock/fallback，未满足阶段门禁时不能进入下一阶段。
