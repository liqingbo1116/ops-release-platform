...
- 如果远程 Linux 主机、二进制启动方式、或平台出站连通性未准备好，Agent 管理无法替换 mock，不能进入阶段 3。
...
- 如果服务或版本来源仍是 mock，发布单创建不能算完成，不能进入阶段 4。
...
- 如果基线来源仍是 mock，基线管理不能算完成，不能进入阶段 5。
...
- 如果 Agent 还不能对真实目标基础设施执行操作，部署执行不能算完成，不能进入阶段 6。
...
- 如果详情页还依赖 mock task 数据，不能进入阶段 7。
...
本节保留为历史上下文，不再作为后续研发执行顺序。当前唯一有效顺序以上文“V1 研发主线路径”为准：环境管理、Agent 管理、发布单创建、基线管理、部署执行、发布/部署详情、登录与权限、清理剩余 mock。每个阶段必须接真实数据并删除该阶段 mock/fallback，未满足阶段门禁时不能进入下一阶段。
