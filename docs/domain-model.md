...
V1 环境管理必须把 K8s、Harbor、Jenkins 作为平台可维护的资源主数据，不把它们隐藏在 `.secrets/` 中作为正式数据来源。同一个 K8s 集群、Harbor 仓库或 Jenkins 实例可以被多个环境复用，同一环境也可以绑定多个作用域。多作用域绑定的业务目的不是“为了绑定而绑定”，而是为后续环境内服务关联做准备：服务需要在某个环境内明确归属到实际使用的 K8s namespace、Harbor project、Jenkins view/job，发布和部署时才能按服务选择正确的构建来源、镜像范围和部署目标。远程环境不由平台直连远程 K8s；但仍需要关联平台侧本地 Jenkins view 和本地 Harbor project，用于触发本地构建、选择镜像来源，并生成后续 Agent 镜像同步与部署任务。
